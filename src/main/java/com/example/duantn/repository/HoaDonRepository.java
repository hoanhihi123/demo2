...
public interface HoaDonRepository extends JpaRepository<HoaDon, UUID> {
    ...
    @Query("SELECT h FROM HoaDon h WHERE " +
            "(:ma is null or h.ma LIKE %:ma%) " +  // Kiểm tra ma nếu không null
            "AND (:startDate is null or h.ngayMua >= :startDate) " + // Kiểm tra ngayMua từ startDate nếu không null
            "AND (:endDate is null or h.ngayMua <= :endDate) " + // Kiểm tra ngayMua đến endDate nếu không null
            "AND (:hinhThucTT is null or h.hinhThucTT = :hinhThucTT or :hinhThucTT = '') " + // Kiểm tra hinhThucThanhToan nếu không null
            "AND (:trangThai is null or h.trangThai = :trangThai or :trangThai = '') ORDER BY h.ma ASC") // Kiểm tra trangThai nếu không null
